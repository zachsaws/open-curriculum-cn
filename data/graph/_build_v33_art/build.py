"""
V3.3.3 Art LLM 化: 78 艺术概念分批调 LLM, 每批 5 + 全面 mechanical repair.

借鉴 english 296 / chinese 209 / physics 121 / labor 85 教训:
- 5 概念/批 (比 30 稳定得多)
- 真换行 → 字面 \\n 必须用 .replace (不用 re.sub)
- post-validation: desc 60-100, ass 150-220, {{name}}=3, \\n>=2, 禁词=0
- 艺术特殊 post-validation (借鉴 english+physics+labor 学到的 LLM bug):
  1. "连续 2+ 空格 + 形式/词" 缺值检测
  2. "用 X 原看明白/掌握/运用/理解" 字符错位检测
  3. "{{name}}能举个例子吗" 模板回潮检测 — 重写到具体场景
- 艺术要"具体作品/具体技法" — 例:
  - 「用红黄蓝三色, X 能不能调出绿色? 试 2 次」
  - 「沙锤摇 8 下, 铃鼓敲 4 下, X 能不能跟着节奏合拍?」
  - 「用水彩笔 3 笔画一棵树, X 能不能画出 3 层不同深浅的绿?」
  - 严禁"培养审美"这种抽象话
- 禁词 0 容忍, 不达标重写

艺术分 6 domain: 音乐/美术/舞蹈/戏剧/影视/综合
"""
import json
import os
import re
...
CACHE = 'data/graph/_build_v33_art/'
...
# 艺术关键词 (用于 "接 X 形式" 缺值检测 — 具体作品/技法/工具/材料/动作/单位)
ART_MISSING = [
    # 作品
...
# ---- prompt 构造 ----
SYSTEM_PROMPT = """你是 V3.3.3 艺术学科内容编辑. 你的任务: 把艺术学科概念用「人话级」中文写出来, 让家长/老师/学生看了就想用.

# description 规则 (60-100 字, 1 段不换行)
- **必须用具体作品/具体技法代替抽象定义** — 例:
  - 音乐: 「跟着《小星星》拍出 XX XX |X X| 的节奏, 每句 4 拍, 强弱强弱, 用手掌拍腿」 — 而不是「感受音乐节奏」
  - 美术: 「用水彩调色盘挤豌豆大的红黄蓝三色, X 能不能两两混出橙绿紫, 试 2 次找出最饱和的比例」 — 而不是「认识三原色」
  - 美术: 「用铅笔起稿勾出树干和 3 根主枝, 再用 3 笔画出叶子, 浓淡墨分 3 层」 — 而不是「中国画基础」
  - 舞蹈: 「左脚迈 1 步重心下压, 右脚跟上抬 30°, 双手举过头顶画 1 个圈, 重复 4 次就是 1 个八拍」 — 而不是「舞蹈基本功」
  - 戏剧: 「把《三只小猪》分 4 个角色, 每人一段台词 5 句话, 加上动作和表情演 5 分钟」 — 而不是「戏剧表演」
  - 影视: 「看一段 3 分钟的动画短片, 找出镜头从远拉近、人物特写、背景音乐变化这 3 个地方」 — 而不是「影视欣赏」
...
- 场景要具体: 含具体作品/具体数字/具体工具/具体动作/具体对话 — 拒绝「理解 X 这一概念, 能否独立完成相关题目?」这种空问
...
def call_llm(concepts: list[dict]) -> str:
    ...
    user_lines = [f'请为以下 {len(concepts)} 个艺术概念各生成 description + assessment_prompt:\n']
    for c in concepts:
        user_lines.append(
            f'---\n'
            f'ID: {c["id"]}\n'
