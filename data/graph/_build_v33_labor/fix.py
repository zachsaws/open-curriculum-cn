"""
V3.3.3 Labor LLM 化: 补 L_L3_02 (简单烹饪)
"""
import json
import os
import re
...
# 通用 prompt (单条)
SYSTEM_PROMPT = """你是 V3.3.3 劳动学科内容编辑. 你的任务: 把劳动学科概念用「人话级」中文写出来.

# description 规则 (60-100 字, 1 段不换行)
- **必须用具体劳动任务代替抽象定义** — 例:
  - 「切土豆丝, 先切片再切丝, 粗细像火柴棍, 一盘菜大概 200 克」
  - 「煮一碗番茄鸡蛋面, 番茄要先炒出汁, 再加水下面, 鸡蛋打散淋上去」
- 劳动要画面感: 工具/材料 + 数字 + 动作 + 看到的现象
- 中间可用「」, **绝不要在 content 内用 ASCII 双引号 "**
- 不用绝对化承诺 (一定/必然/肯定)
- 反直觉 + 具体, 优于课标原文
- **绝不要"培养劳动习惯"这种抽象话** — 一定要落到具体动作

# assessment_prompt 规则 (150-220 字, 3 问)
- **正好 3 问**, 行间用 \\n 分隔 (一个反斜杠加 n, 2 字符)
- 每问**正好 1 个** {{name}} 占位符 (两个花括号包 name, 不能用「小明」「孩子」等替代)
- 场景要具体: 含具体数字/具体工具/具体动作/具体对话
- 3 问难度递进: 第 1 问直接识别/操作, 第 2 问反例/纠错, 第 3 问解释/迁移
- 中文要自然: "能不能 / 会不会" 优于 "能否"
- 劳动优先具体场景: "用 1 把刀切 1 个土豆, {{name}} 能不能切成 3 mm 厚的片?"
